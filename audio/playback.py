...
class PlaybackWorker(QThread):
    ...
    def __init__(
        ...
        self._prev_chunk_tail = np.array([], dtype=np.float32)
    ...
    def _audio_callback(self, outdata, frames, time_info, status):
        ...
        with self._buffer_lock:
            if len(self._buffer) >= frames:
                ...
                outdata[len(self._buffer) :, 0] = np.zeros(
                    frames - len(self._buffer), dtype=np.float32
                )
                self._buffer = np.array([], dtype=np.float32)
            else:
                outdata[:] = np.zeros((frames, 1), dtype=np.float32)
...
class PlaybackManager(QObject):
    ...
    def play(self, start_index: int = 0):
        ...
        self._worker = PlaybackWorker(self._audio_queue, 22050, crossfade_ms=8)
        ...
        self._synthesis_thread = threading.Thread(
            target=self._synthesize_sentences, daemon=True
        )
